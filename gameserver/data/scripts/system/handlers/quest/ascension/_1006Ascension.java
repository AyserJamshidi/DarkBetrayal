...
public class _1006Ascension extends QuestHandler {
                ...
                case 790001: { // Pernos
                    switch (dialog) {
                        ...
                        case STEP_TO_1: {
                            ...
                            TeleportService.teleportTo(player, 210010000, 657f, 1071f, 99.375f, (byte) 72, TeleportAnimation.BEAM_ANIMATION);
                            return true;
                        }
                        case STEP_TO_3: {
                            WorldMapInstance newInstance = InstanceService.getNextAvailableInstance(310010000);
                            InstanceService.registerPlayerWithInstance(newInstance, player);
                            TeleportService.teleportTo(player, 310010000, newInstance.getInstanceId(), 52f, 174f, 229f);
                            ...
                            if (qs.getQuestVars().getQuestVars() == 99) {
                                ...
                                ThreadPoolManager.getInstance().schedule(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        mobs.add((Npc) QuestService.addNewSpawn(310010000, instanceId, 211042, (float) 224.073, (float) 239.1, (float) 206.7,
                                            (byte) 0));
                                        mobs.add((Npc) QuestService.addNewSpawn(310010000, instanceId, 211042, (float) 233.5, (float) 241.04, (float) 206.365,
                                            (byte) 0));
                                        mobs.add((Npc) QuestService.addNewSpawn(310010000, instanceId, 211042, (float) 229.6, (float) 265.7, (float) 205.7,
                                            (byte) 0));
                                        mobs.add((Npc) QuestService.addNewSpawn(310010000, instanceId, 211042, (float) 222.8, (float) 262.5, (float) 205.7,
                                            (byte) 0));
                                        for (Npc mob : mobs) {
        ...
        if (qs != null && qs.getStatus() == QuestStatus.START) {
            ...
            if (var >= 51 && var < 54) {
                ...
                Npc mob = (Npc) QuestService
                    .addNewSpawn(310010000, player.getInstanceId(), 211043, (float) 226.7, (float) 251.5, (float) 205.5, (byte) 0);
                mob.getAggroList().addDamage(player, 1000);
                return true;
            }
        }
    ...
    public boolean onMovieEndEvent(QuestEnv env, int movieId) {
        ...
        if (movieId == 151) {
            ...
            qs.setQuestVar(3);
            updateQuestStatus(env);
            TeleportService.teleportTo(player, 210010000, 246f, 1639f, 100.316f, (byte) 56, TeleportAnimation.BEAM_ANIMATION);
            return true;
        }
        return false;
    }

...
public class _2059APeaceOffering extends QuestHandler {
	...
	public boolean onNpcReachTargetEvent(QuestEnv env) {
		...
		return defaultFollowEndEvent(env, 3, 4, false);
	}
			...
			switch (env.getDialog()) {
				...
				case STEP_TO_3:
					if (var == 2) {
						...
						updateQuestStatus(env);
						player.sendPck(new SM_DIALOG_WINDOW(env.getVisibleObject().getObjectId(), 10));
						Npc survivor = (Npc) QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 204806, player.getX(), player.getY(), player.getZ(), (byte) 0);

...
public class _89999ItemGiving extends QuestHandler {
    ...
    public boolean onDialogEvent(QuestEnv env) {
        ...
        switch (env.getDialog()) {
            case USE_OBJECT:
                player.sendPck(new SM_DIALOG_WINDOW(targetId, 1011, 0));
                return true;
            case SELECT_ACTION_1012: {
                Storage inventory = player.getInventory();
                if (inventory.getItemCountByItemId(itemId) > 0) {
                    player.sendPck(new SM_DIALOG_WINDOW(targetId, 1097, 0));
                    return true;
                } else {
                    if (giveQuestItem(env, itemId, 1)) {
                        player.sendPck(new SM_DIALOG_WINDOW(targetId, 1012, 0));
                    }
                    return true;
                }
            }

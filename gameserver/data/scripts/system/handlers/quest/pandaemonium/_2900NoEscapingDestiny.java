...
public class _2900NoEscapingDestiny extends QuestHandler {
            ...
            switch (targetId) {
                    ...
                    switch (dialog) {
                        ...
                        case STEP_TO_5: {
                            if (var == 4) {
                                ...
                                TeleportService.teleportTo(player, 320070000, newInstance.getInstanceId(), 268.47397f, 251.80275f, 125.8369f);
                                return closeDialogWindow(env);
                            }
                        }
                        case STEP_TO_9: {
                            changeQuestStep(env, 9, 10, false); // 10
                            TeleportService.teleportTo(player, 220010000, 1, 383.0f, 1896.0f, 327.625f);
                            return closeDialogWindow(env);
                        }
                    }
                    break;
                ...
                case 204264: { // Skuld 2
                    switch (dialog) {
                        ...
                        case SELECT_ACTION_3058: {
                            if (var == 96) {
                                if (giveQuestItem(env, getStoneId(player), 1) && !isStigmaEquipped(env)) {
                                    ...
                                    if (existendStigmaShards < 60 && !petWarehouse(env)) {
                        ...
                        case STEP_TO_7: {
                            if (var == 99) {
                                player.sendPck(new SM_DIALOG_WINDOW(env.getVisibleObject().getObjectId(), 1));
                                return true;
                            }
                        }
                        case STEP_TO_8: {
                            if (var == 97) {
                                ...
                                QuestService.addNewSpawn(320070000, player.getInstanceId(), 204263, 257.5f, 245f, 125.8369f, (byte) 0);
                                return closeDialogWindow(env);
                            }
                        }
                    }
    ...
    private boolean petWarehouse(QuestEnv env) {
        Player player = env.getPlayer();
        for (Storage storage : player.getPetBags()) {
            if (storage.hasItem(141000001))
                return true;
        }
        return false;
    }
    ...
    public boolean onKillEvent(QuestEnv env) {
        ...
        if (qs != null && qs.getStatus() == QuestStatus.START) {
            ...
            if (var == 98) {
                ...
                TeleportService.teleportTo(player, 220010000, 1, 1113.8882f, 1719.497f, 271.07687f);
                return true;
            }
        }
        return false;

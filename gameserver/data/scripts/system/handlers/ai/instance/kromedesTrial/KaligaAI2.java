...
@AIName("кaliga")
public class KaligaAI2 extends AggressiveNpcAI2 {
    ...
    protected void handleAttack(Creature creature) {
		...
		if (stage == 1) {
			...
			ThreadPoolManager.getInstance().schedule(new Runnable() {
				...
				@Override
				public void run() {
					SkillEngine.getInstance().getSkill(getOwner(), 19787, 38, getOwner()).useSkill();
					SkillEngine.getInstance().getSkill(getOwner(), 19788, 38, getOwner()).useSkill();
				}
			}, 2000);
		}
    }
    ...
    public void handleBackHome() {
		...
		stage = 0;
        getEffectController().removeEffect(19787);
        getEffectController().removeEffect(19788);
		addPercent();
		curentPercent = 100;

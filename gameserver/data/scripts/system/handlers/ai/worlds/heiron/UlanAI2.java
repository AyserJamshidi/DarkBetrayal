...
@AIName("ulan")
public class UlanAI2 extends AggressiveNpcAI2 {
	...
	private void checkPercentage(int hpPercentage) {
        if (hpPercentage <= 75) {
			...
			if (stage1 == 1) {
				helpspawn();
				SkillEngine.getInstance().getSkill(getOwner(), 17861, 40, getOwner()).useNoAnimationSkill();
			}
        }
        if (hpPercentage <= 50) {
			stage2++;
	...
	private void helpspawn() {
		...
		attackPlayer((Npc) spawn(280806,  p.getX() + x1, p.getY() + y1, p.getZ(), (byte)0));
		attackPlayer((Npc) spawn(280806,  p.getX() + x1, p.getY() + y1, p.getZ(), (byte)0));
		attackPlayer((Npc) spawn(280806,  p.getX() + x1, p.getY() + y1, p.getZ(), (byte)0));
    }

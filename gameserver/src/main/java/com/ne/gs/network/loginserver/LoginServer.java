...
import com.ne.gs.network.aion.serverpackets.SM_L2AUTH_LOGIN_CHECK;
import com.ne.gs.network.aion.serverpackets.SM_RECONNECT_KEY;
import com.ne.gs.network.loginserver.LoginServerConnection.State;
import com.ne.gs.network.loginserver.serverpackets.SM_ACCOUNT_AUTH;
import com.ne.gs.network.loginserver.serverpackets.SM_ACCOUNT_DISCONNECTED;
import com.ne.gs.network.loginserver.serverpackets.SM_ACCOUNT_RECONNECT_KEY;
import com.ne.gs.network.loginserver.serverpackets.SM_BAN;
import com.ne.gs.network.loginserver.serverpackets.SM_LS_CONTROL;
import com.ne.gs.services.AccountService;
...
public class LoginServer {
    ...
    /**
     * This method is called when we lost connection to LoginServer. We will disconnects all aionClients waiting for LoginServer response and also try reconnect
     * to LoginServer.
     */
    ...
    /**
     * Notify that client is disconnected - we must clear waiting request to LoginServer if any to prevent leaks. Also notify LoginServer that this account is
     * no longer on GameServer side.
     *
     * @param accountId
     */
    ...
    public void accountAuthenticationResponse(int accountId, String accountName, boolean result, AccountTime accountTime,
                                              byte accessLevel, byte membership, @Deprecated long toll, long expire) {
        ...
        Account account = AccountService.getAccount(accountId, accountName, accountTime, accessLevel, membership, toll, expire);

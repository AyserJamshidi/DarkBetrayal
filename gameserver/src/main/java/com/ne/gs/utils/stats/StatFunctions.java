...
public final class StatFunctions {
     ...
     * @return int
     */
    public static int calculateMagicalResistRate(Creature attacker, Creature target, int accMod) {
        if (target.getObserveController().checkAttackStatus(AttackStatus.RESIST)) {
            return 1000;
        }

        int mResist = target.getGameStats().getMResist().getCurrent();
        int mAcc = attacker.getGameStats().getMAccuracy().getCurrent();
        int levelMod = target.getLevel() - attacker.getLevel() - 2;

        mAcc += accMod;

        mAcc = Math.max(1, mAcc);
        mResist = Math.max(1, mResist);
        levelMod = XMath.limit(0, levelMod, 80);

        int resistRate = mResist - mAcc;
        resistRate += levelMod * 100;

        resistRate = XMath.limit(1, resistRate, 950);

...
public abstract class ChatCommandHandler implements Filter<String> {
    ...
    private final ChatCommandSecurity _security;

    protected ChatCommandHandler(ChatCommandRegistry registry, ChatCommandSecurity security) {
        _registry = registry;
        _security = security;
    }
    ...
    public ChatCommandSecurity getSecurity() {
        return _security;
    }
    ...
    protected boolean run(Player player, String message) {
        ...
        return getSecurity().check(playerName, className, player.getAccessLevel()) && cmd.run(player, alias, params);
    }

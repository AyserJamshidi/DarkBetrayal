...
public abstract class EffectTemplate {
    ...
    public boolean calculate(Effect effect, StatEnum statEnum, SpellStatus spellStatus, boolean ignoreResist) {
        ...
        if(!(this instanceof StunEffect)) {
            //if all preeffects was success, this effect can't be resisted
            FastList<Integer> pEffects = getPreEffects();
            if (pEffects.size() > 0) {

                boolean suc = true;

                for (int pEffect : pEffects) {
                    if (!effect.isInSuccessEffects(pEffect)) {
                        suc = false;
                        break;
                    }
                }

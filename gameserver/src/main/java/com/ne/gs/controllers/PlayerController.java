...
public class PlayerController extends CreatureController<Player> {
    ...
    private final PlayerDieHandler DEFAULT_PLAYER_DIE_HANDLER = new DefaultPlayerDieHandler();
    ...
    public void setOwner(Creature owner) {
        if (getOwner() != null) {
            getOwner().getChainer().detach(DEFAULT_PLAYER_DIE_HANDLER);
            getOwner().getChainer().detach(EffectResurrectBaseHandler.STATIC);
        }
        super.setOwner(owner);
        if (getOwner() != null) {
            getOwner().getChainer().attach(DEFAULT_PLAYER_DIE_HANDLER);
    ...
    public void onDie(@NotNull Creature lastAttacker, boolean showPacket) {
        ...
        getOwner().getChainer().handle(PlayerDieHandler.class, Tuple3.of(getOwner(), lastAttacker, showPacket));
    ...
    private void sendDieFromCreature(@NotNull Creature lastAttacker, boolean showPacket) {
        ...
        PacketSendUtility.broadcastPacket(player, new SM_EMOTION(player, EmotionType.DIE, 0, player.equals(lastAttacker) ? 0 : lastAttacker
            .getObjectId()),
            true);
        if (showPacket) {
    ...
    public void startProtectionActiveTask() {
        if (!getOwner().isProtectionActive()) {
            ...
            }, CustomConfig.SPAWN_PROTECTION_TIME * 1000);
    ...
    public boolean isInCombat() {
        long now = Sys.millis();
        return (((now - lastAttackedMilis) <= 10000) || ((now - lastAttackMilis) <= 10000));
    }
    ...
    private class DefaultPlayerDieHandler extends PlayerDieHandler {
        @Override
        public Boolean onEvent(@NotNull Tuple3<Player, Creature, Boolean> e) {
            Player player = e._1;
            Creature lastAttacker = e._2;
            boolean showPacket = e._3;

            showPacket = !player.hasResurrectBase() && showPacket;
            Creature master = lastAttacker.getMaster();

            if ((PvPConfig.ENABLE_KILLING_SPREE_SYSTEM) && (player.getRawKillCount() > 0)) {
